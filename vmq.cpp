...
// Modified 2017 by Alexander Altman for modern C++
...
#ifdef __AMIGA__
__attribute__((unused)) static char const *VersionString =
    "$VER: " VERSION "(" __DATE__ ")";
#else
__attribute__((unused)) static char const *VersionString =
    VERSION " (" __DATE__ ")";
#endif

static char const *Copyright = "Copyright 2002 Raymond L. Zarling";
...
public:
  qfreader(istream &is, storage_type &mem, vector<quad_type> &qlist)
      : m_errorlevel(0), m_lineno(0), m_is(is), m_mem(mem), m_qlist(qlist) {}
  int go(void);
...
public:
  interpreter(storage_type &mem, const vector<quad_type> &qlist)
      : m_mem(mem), m_qlist(qlist), m_tracing(false), m_errorlevel(0) {}
  int go(void);
...
int main(int argc, char *argv[]) {
  // Identify
  cerr << "Virtual Quadrule Machine version " << VERSION << " (" << __DATE__
       << ")" << endl;
  cerr << Copyright << endl;
  ...
  case 2: {
    ...
    if (!qf) {
      ...
      // what went wrong
      cerr << "Can't open file " << argv[1] << ": " << strerror(errno) << endl;
      exit(10);
    }
...
int qfreader::go(void) {
  ...
  for (; m_is; getline(m_is, m_line)) {
...
#endif
    if (datasection) {         /* use the line contents to initialize some data storage
                                */
      ...
      } else // not yet end of datasection
      {
        ...
        if (space == string::npos ||
            (vptr = m_line.find_first_not_of(" \t", space)) == string::npos) {
        ...
        case '\"': // string constant
          try {
            ...
            m_mem.Set(a, m_line.substr(vptr + 1, end - vptr - 1).c_str());
        ...
        case '.':
          try {
            string::size_type x = m_line.find_first_of(". \t\n", vptr);
            if (m_line[x] == '.') {
              float f;
              ss >> f;
              m_mem.Set(a, f);
    ...
    case '$':
      ss >> sop >> sop1 >> sop2;
      m_qlist.push_back(quad_type(sop, parse_short(sop1), parse_short(sop2)));
      break;
...
qop qfreader::parse_adr(const string &s, bool dst, bool flt) const {
  ...
  posterror(ERR_FATAL, "Impossible case");
...
int interpreter::go(void) {
  ...
  try {
    while (!stop) {
      ...
      if (m_qlist[m_pc].tron())
        m_tracing = true;
      if (m_qlist[m_pc].troff())
        m_tracing = false;
      ...
      if (m_tracing)
        cout << setw(4) << m_pc << ": " << m_qlist[m_pc]; // Do endl later...
      ...
      case '&':
        ...
        switch (cur_op) {
        case 'a':
          m_mem.Set(res_adr, short(op1.sval(m_mem) + op2.sval(m_mem)));
          res_type = 's';
          break;
        case 'A':
          m_mem.Set(res_adr, float(op1.fval(m_mem) + op2.fval(m_mem)));
          res_type = 'f';
          break;
        case 's':
          m_mem.Set(res_adr, short(op1.sval(m_mem) - op2.sval(m_mem)));
      ...
      case 'N':
        ...
        switch (cur_op) {
        case 'i':
          m_mem.Set(res_adr, op1.sval(m_mem));
          res_type = 's';
          break;
        case 'I':
          m_mem.Set(res_adr, op1.fval(m_mem));
          res_type = 'f';
          break;
        case '=':
          m_mem.Set(res_adr, char(op1.cval(m_mem)));
          res_type = 'c';
          break;
        case 'F':
          m_mem.Set(res_adr, float(op1.sval(m_mem)));
          res_type = 'f';
          break;
        case 'f':
          m_mem.Set(res_adr, short(op1.fval(m_mem)));
          res_type = 's';
          break;
        case '~':
          m_mem.Set(res_adr, short(~op1.sval(m_mem)));
          res_type = 's';
          break;
        case 'n':
          m_mem.Set(res_adr, short(-op1.sval(m_mem)));
          ...
          break;
        case 'N':
          m_mem.Set(res_adr, float(-op1.fval(m_mem)));
          res_type = 'f';
          break;
        } // end switch
        if (m_tracing)
      ...
      case 'c':
        ...
        } else // Pseudo-function call
        {
          ...
          default:
            posterror(ERR_ERROR, "Unrecognized pseudo-quad number: STOP");
            return m_errorlevel;
          } // end switch
      ...
      case 'P':
        ...
        if (m_tracing) {
          ...
          adr_type i;
          cout << " --> stack now";
          for (i = m_mem.STop(); i <= m_mem.DLink() && i < m_mem.STop() + 16;
               i += 2) {
      ...
      case '$':
        if (running) {
          posterror(ERR_ERROR, "'$' Quad may only be executed once: STOP");
          return m_errorlevel;
        }
        running = true;
        op1 = m_qlist[m_pc].op1();
      ...
      case '^':
        ...
        if (cur_op == '^') // pop runtime stack
        {
          ...
          if (n & 0x0001) {
            posterror(ERR_ERROR, "Must pop an even number of bytes: STOP");
            return m_errorlevel;
          }
          m_mem.Pop(n);
          if (m_tracing) {
...
void interpreter::traceresult(adr_type res_adr, char res_type) {
  ...
  switch (res_type) {
  case 'a':
    cout << setw(6) << m_mem.Adr(res_adr);
    break;
  case 's':
    cout << setw(6) << m_mem.Short(res_adr) << dec
         << " ( = " << m_mem.Short(res_adr) << " )";
    break;
  case 'c':
    cout << /*setw(4) <<*/ (unsigned short)m_mem.Char(res_adr);
    if (isprint(m_mem.Char(res_adr)))
      cout << " ( = " << m_mem.Char(res_adr) << " )";
    break;
  case 'f':
    cout << m_mem.Float(res_adr);
    break;
  }
  cout.fill(oldfill);
  cout.flags(fmt);
