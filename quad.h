...
struct qop {
  ...
  qop(short v, char a = ' ', char vt = 's') : val(v), vtype(vt), adrmode(a) {}
  qop(adr_type v, char a = ' ', char vt = 'a')
      : val(v), vtype(vt), adrmode(a) {}
  qop(float v, char a = ' ', char vt = 'f') : val(v), vtype(vt), adrmode(a) {}
  ...
  inline adr_type lval(const storage_type &mem) throw(runtime_error) {
    ...
    case '#':
    case 'M':
      throw runtime_error("Illegal use of l-value");
    case '_':
      return adr_type(mem.DLink() + val.s);
    case 'N':
      return mem.Adr(adr_type(mem.DLink() + val.s));
    case ' ':
      return val.a;
    case '@':
      return mem.Adr(val.a);
    }
  ...
  inline short ival(__attribute__((unused))
                    const storage_type &mem) throw(runtime_error) {
    switch (adrmode) {
    case '#':
    case 'M':
    case '_':
  ...
  short sval(const storage_type &mem) {
    switch (adrmode) {
    case 'M':
      return short(mem.DLink() + val.s);
    case '#':
      return short(val.s);
    case '_':
      return mem.Short(adr_type(mem.DLink() + val.s));
    case ' ':
      return mem.Short(val.a);
    case 'N':
      return mem.Short(mem.Adr(adr_type(mem.DLink() + val.s)));
    case '@':
      return mem.Short(mem.Adr(val.a));
    }
    throw runtime_error("Impossible case");
  }
  ...
  short cval(const storage_type &mem) {
    switch (adrmode) {
    case 'M':
      return char(mem.DLink() + val.s);
    case '#':
      return char(val.s);
    case '_':
      return mem.Char(adr_type(mem.DLink() + val.s));
    case ' ':
      return mem.Char(val.a);
    case 'N':
      return mem.Char(mem.Adr(adr_type(mem.DLink() + val.s)));
    case '@':
      return mem.Char(mem.Adr(val.a));
    }
    throw runtime_error("Impossible case");
  }
  ...
  float fval(const storage_type &mem) {
    ...
    case 'M':
      return val.f;
    case '_':
      return mem.Float(adr_type(mem.DLink() + val.s));
    case ' ':
      return mem.Float(val.a);
    case 'N':
      return mem.Float(mem.Adr(adr_type(mem.DLink() + val.s)));
    case '@':
      return mem.Float(mem.Adr(val.a));
    }
    throw runtime_error("Impossible case");
  }
  ...
  adr_type aval(const storage_type &mem) {
    ...
    case ' ':
      return mem.Adr(val.a);
    case 'N':
      return mem.Adr(mem.Adr(adr_type(mem.DLink() + val.s)));
    case '@':
      return mem.Adr(mem.Adr(val.a));
    }
    throw runtime_error("Impossible case");
  }
...
public:
  quad_type(char op = '\0', qop o1 = qop(), qop o2 = qop(), qop o3 = qop())
      : m_op(op), m_o1(o1), m_o2(o2), m_o3(o3), m_tron(false), m_troff(false),
        m_dump(false) {}
...
ostream &operator<<(ostream &os, const quad_type &q) {
  ...
  if (q.m_op) {
    ...
    case ';':
      break;
    default:
      os << "Invalid operation: 0x" << setw(2) << hex << int(q.m_op);
    }
    os.flags(fmt);
  } else // !q.m_op
    os << "uninitialized";
