...
public:
  ...
  inline char Char(const adr_type adr) const { return *l_byte(adr); };
  inline short Short(const adr_type adr) const { return *l_short(adr); };
  inline adr_type Adr(const adr_type adr) const { return *l_adr(adr); };
  inline float Float(const adr_type adr) const {
    float f;
    memcpy(&f, l_float(adr), sizeof(float));
    return f;
  }
  inline char *Str(const adr_type adr) { return l_str(adr); };

  //		Functions to write to emulated memory
  inline void Set(const adr_type adr, const char val) { *l_byte(adr) = val; };
  inline void Set(const adr_type adr, const short val) { *l_short(adr) = val; };
  ...
  inline adr_type Pop_Adr(void) {
    adr_type result = *l_adr(m_top);
    m_top += sizeof(adr_type);
    return result;
  }
  inline short Pop_Short(void) {
    short result = *l_short(m_top);
    m_top += sizeof(short);
    return result;
  }
  ...
  void Dump(const adr_type adr1, const adr_type adr2,
    ...
    for (adr_type a = adr1; a <= adr2; a++) {
      ...
      //			cout << setw(2)
      //				<< (((unsigned short)m_store[a]) &
      // 0xff);
      //	However, g++ 3.2 on Sun produces a program that segmentation
      // faults
...
private:
  ...
  inline void *l_float(const adr_type adr) const {
    check_align(adr, 2);
    return (void *)&m_store[adr];
  };
  // "str" is for character strings, terminated by '\0'
  inline char *l_str(const adr_type adr) { return &m_store[adr]; }
  ...
  void check_align(const adr_type adr, const int mult) const
    ...
    string msg = "Unaligned data access, expecting multiple of ";
    msg += name[mult];
    if (adr & (mult - 1)) {
